...
return (
    ...
    // <div key={index} className="flex flex-col justify-center items-center rounded-lg  h-[200px] w-[400px] p-4">
    <div  key={index} className=" flex flex-col p-4 h-[160px] w-[400px] bg-white justify-around">
      <h1 className='text-myorange text-2xl'>{item.name}</h1>
      <p className='text-gray-600 text-lg'>type:{item.type}</p>
      <p className={`text-black text-lg ${item.available ? "text-green" : "text-red"}`}>{item.available ? "Available" : "Not Available"}</p>
    </div>
  // </div>
  ))}
</div>
